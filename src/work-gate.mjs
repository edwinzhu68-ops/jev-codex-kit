...
function assignmentText(a) {
  ...
  if (!['spawn_agent', 'followup_task', 'send_input', 'send_message', 'send_message_to_thread'].includes(name)) return null;
...
export async function reviewWork(raw, { home = kitHome() } = {}) {
  ...
  return locked(home, input.session_id, async () => {
    ...
    const assignments = input.actions.filter(a => assignmentText(a) !== null);
...
function collectionCall(event) {
  if (['wait', 'wait_agent', 'list_agents', 'get_goal', 'multi_agent_v1wait_agent', 'multi_agent_v1list_agents'].includes(event.tool_name)) return true;
