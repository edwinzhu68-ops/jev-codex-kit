...
旧 `workflow` 模式仍是零模型提醒，不是执行检查；现已移除该本地提醒对长消息、代码块和“继续”的跳过。付费技能路由保持原有保守过滤。0.4.2 的[工作量对照](docs/evaluations/workload-benefit-20260922.md)中，8 条中文问题分类约 0.69 秒、8/8 符合预设答案，但源码声明核对仍有错误和大量待复核项。证据文本保留完整原文和判断；不据此宣称整项编程任务提速。
