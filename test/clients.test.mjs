...
test('Pi native adapter registers 11 tools and performs actual MCP evidence/status calls without inference',async()=>{
  ...
  try{
    assert.equal(tools.size,11);assert.equal(tools.get('jev_prepare_evidence').parameters.type,'object');
    await commands.get('jev-status').handler();assert.match(messages[0].msg.content,/11 tools connected/);assert.equal(messages[0].opts.triggerTurn,false);
